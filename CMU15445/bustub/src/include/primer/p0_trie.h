...
class TrieNode {
 public:
  ...
  bool HasChild(char key_char) const {
    // 使用迭代器算法
    return this->children_.find(key_char) != children_.end();
  }
  ...
  void SetChar(const char &new_char) { this->key_char_ = new_char; }
...
class Trie {
 ...
 public:
   ...
   */
  Trie() {
    auto *node = new TrieNode('\0');
    // unique_ptr只能重新指向指针，不能赋值
  ...
  bool Insert(const std::string &key, T value) {
    ...
    // 直接在root下插入新结点
    latch_.WLock();
    auto *pre_node = &this->root_;
    for (auto &ch : key) {
      if (pre_node->get()->GetChildNode(ch) == nullptr) {
        // 将这个结点插入自己的map后指向孩子结点
        pre_node = pre_node->get()->InsertChildNode(ch, std::make_unique<TrieNode>(TrieNode(ch)));
        continue;
      }
      // 向下一个孩子结点走
      pre_node = pre_node->get()->GetChildNode(ch);
    }

    if (!pre_node->get()->IsEndNode()) {
      *pre_node = std::unique_ptr<TrieNode>(new TrieNodeWithValue<T>(std::move(*(pre_node->get())), value));
      latch_.WUnlock();
      return true;
    }
    latch_.WUnlock();
  ...
  bool Remove(const std::string &key) {
    ...
    // 递归删除结点函数
    RemoveNode(key, 0, &this->root_);
    latch_.WUnlock();
    return true;
  }

  // 递归删除结点
  void RemoveNode(const std::string &key, std::string::size_type index, std::unique_ptr<TrieNode> *parent) {
    if (index == key.size()) {
      return;
    }
    auto *child_node = parent->get()->GetChildNode(key[index]);
    if (child_node == nullptr) {
      return;
    }
    RemoveNode(key, index + 1, child_node);

    if (!child_node->get()->HasChildren()) {
      parent->get()->RemoveChildNode(key[index]);
    }
  }
  ...
  T GetValue(const std::string &key, bool *success) {
    *success = false;
    ...
    auto *res = dynamic_cast<TrieNodeWithValue<T> *>(pre_node->get());
    if (res == nullptr) {
      latch_.RUnlock();
      return {};
    }
